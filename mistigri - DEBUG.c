...
void destroy_pair(hands j, int p) {
    ...
    while (true) {
        ct = 0;
        for (int i = 0; i < j[p].nb_cards; i++) {
            for (int k = 0; k < j[p].nb_cards; k++) {
                if (k != i && abs(j[p].cards[i]) == abs(j[p].cards[k]) && j[p].cards[i] != 0) {
                    remove_pair(j, p, i, k);
                    ct++;
                    break;
                }
...
void play(hands jeu, int player, int nb, int *ids) {
    if (jeu[ids[player]].nb_cards != 0 && nb != 1) {
        ...
        DEBUG ? printf("Pioche le %d du joueur %d\n", carte, joueur + 1) : NULL;
...
void stats(int N, int nb_game, int nb_player) {
    ...
    int *ratio = malloc(sizeof(int) * nb_player);
    for (int i = 0; i < nb_player; i++) {
        ratio[i] = 0;
    }

    deck cartes = raw_deck(N);
    DEBUG ? printf("Paquet non trie : ") : NULL;
    DEBUG ? print_cards(cartes, 2 * N + 1) : NULL;
    ...
    for (int game = 0; game < nb_game; game++) {
        ...
        shuffle(2 * N + 1, cartes);
        DEBUG ? printf("Paquet trie : ") : NULL;
        DEBUG ? print_cards(cartes, 2 * N + 1) : NULL;

        hands jeu = distribue(2 * N + 1, cartes, nb_player);

        DEBUG ? printf("Mains de depart : ") : NULL;
        DEBUG ? print_hands(jeu, nb_player) : NULL;
        ...
        while (player_in_game > 1) {
            for (int player = 0; player < player_in_game; player++) {
                DEBUG ? printf(" -- Joueur n%d : \n", ids[player] + 1) : NULL;
                DEBUG ? print_cards(jeu[ids[player]].cards, jeu[ids[player]].nb_cards) : NULL;
                print_cards(ids, player_in_game);
